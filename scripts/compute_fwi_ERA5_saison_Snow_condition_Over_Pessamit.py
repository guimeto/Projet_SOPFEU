...
for year in range(yi,yf+1):
    ...
    multi_pr = [f'{file}{month}.nc' for month in ['03','04','05','06','07','08']]
    # variables humidité, temperature et vent à 17h UTC
    file = 'J:/REANALYSES/ERA5/UU_VV_TC/ERA5_QC_TC_WIND_'+str(year)
    multi_air = [f'{file}{month}_17h.nc' for month in ['03','04','05','06','07','08']]
    # humidity
    file = 'J:/REANALYSES/ERA5/HR/ERA5_QC_Humidity_'+str(year)
    multi_hum = [f'{file}{month}_17h.nc' for month in ['03','04','05','06','07','08']]
    # épaisseur de neige quotidienne 
    multi_snow = ['J:/REANALYSES/ERA5/SWE/ERA5_'+str(year)+month+'/ERA5_SWE_'+str(year)+month+'_daymean.nc4' for month in ['03','04','05','06','07','08']]
    ...
    IND2 = np.zeros((len(prec[0]),len(prec[0][0])),dtype=float)
    ...
    data_onset = xr.Dataset( coords={'lon': ([ 'lon'], ds1.variables['longitude'][:]),
                                     'lat': (['lat',], ds1.variables['latitude'][:])})
    data_onset["Onset"] = (['lat', 'lon'],  IND2)
    data_set["FWI"] = (['time','lat', 'lon'],  IND)
    data_set = data_set.assign_coords(lon=(((data_set.lon + 180) % 360) - 180)).sortby('lon')  
    data_onset = data_onset.assign_coords(lon=(((data_onset.lon + 180) % 360) - 180)).sortby('lon') 
    
    ## load mask 
    mask = xr.open_mfdataset('K:/PROJETS/PROJET_FIRE_INDEX/scripts/mask_Nitassinan.nc')
    data_set=data_set.where(mask.region == 0)
    data_onset=data_onset.where(mask.region == 0)
    del [ prec, hum, wind, temp, lats, lons] 
 
    # ecriture du Netcdf
    data_set.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_FWI_Netcdf/ERA5_FWI_Pessamit_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    data_onset.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_Onset_Netcdf/ERA5_SNOW_Onset_Pessamit_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    
    # compute of severity rating SR = 0.0272 * FWI^1.77 
    SR = 0.0272 * np.power(data_set, 1.77)
    SR = SR.rename({'FWI':'SR'})
    
    SR.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_SR_Netcdf/ERA5_SR_Pessamit_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    ...
    SR = SR.sum(dim='time')
    SR.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_SR_Netcdf/ERA5_cumulate_SR_Pessamit_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
