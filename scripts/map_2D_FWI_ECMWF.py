...
## Date à utiliser 
mask = xr.open_mfdataset('K:/PROJETS/PROJET_FIRE_INDEX/Pyush/ERA5_mask_QUEBEC.nc')
mask=mask.rename({'Longitude': 'longitude','Latitude': 'latitude'})
dataset = xr.open_mfdataset('K:/PROJETS/PROJET_FIRE_INDEX/FWI_ERA5_ECMWF/ECMWF_FWI_FWI_20180801_1200_hr_v3.1_con.nc')
dataset = dataset.assign_coords(longitude=(((dataset.longitude + 180) % 360) - 180)).sortby('longitude')

dataset=dataset.where(mask.region == 1)
dataset.to_netcdf('./tmp.nc')
...
data_set.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_FWI_Netcdf/ERA5_ECMWF_FWI_QC_'+dtime[index_to_find].strftime("%Y-%m-%d")+'.nc')
...
string_title=u'Fire Weather Index ECMWF server: '+dtime[index_to_find].strftime("%Y-%m-%d") 
plt.title(string_title, size='xx-large')
plt.savefig('./figures/FWI_ECMWF_'+dtime[index_to_find].strftime("%Y-%m-%d")+'.png', bbox_inches='tight', pad_inches=0.1)
plt.show()  
plt.close()
