...
for year in range(yi,yf+1):
    ...
    data_onset=data_onset.where(mask.region == 1)
    ...
    data_onset.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_Onset_Netcdf/ERA5_SNOW_Onset_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    
    # compute of severity rating SR = 0.0272 * FWI^1.77 
    SR = 0.0272 * np.power(data_set, 1.77)
    SR = SR.rename({'FWI':'SR'})
    
    SR.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_SR_Netcdf/ERA5_SR_QC_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    ...
    SR = SR.sum(dim='time')
    SR.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_SR_Netcdf/ERA5_cumulate_SR_QC_SNOW_New_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
