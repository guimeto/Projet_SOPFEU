...
for year in range(yi,yf+1):
    ...
    file = 'J:/REANALYSES/ERA5/HR/ERA5_QC_TC_WIND_'+str(year)
    multi_air = [f'{file}{month}_17h.nc' for month in ['04','05','06','07','08']]
    
    file = 'J:/REANALYSES/ERA5/era5_edna_ea_HR_'+str(year)
    multi_hum = [f'{file}{month}_1000hpa_17h.nc' for month in ['04','05','06','07','08']]
    ...
    rhum = xr.open_mfdataset(multi_hum)
    ...
    hum =  rhum.sel(longitude=slice(*lon_bnd), latitude=slice(*lat_bnd)).r
    ...
    DS = xr.merge([prec, wind,temp])
    DS.to_netcdf('./tmp.nc')
    del [prec, wind,  temp, DS] 
    ds1 = Dataset('./tmp.nc')
    ...
    # ecriture du Netcdf
    data_set.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_FWI_Netcdf/ERA5_FWI_QC_SNOW3_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
    ...
    SR = SR.sum(dim='time')
    SR.to_netcdf('K:/PROJETS/PROJET_FIRE_INDEX/ERA5_FWI_Netcdf/ERA5_SR_QC_SNOW3_'+str(year)+'_from_'+str(dtime[0].month)+'_to_'+str(dtime[nt].month)+'.nc')
